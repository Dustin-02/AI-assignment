...
def collate_superv(batch, max_len):
    """Custom collate function for supervised learning."""
    
    # Unzip the batch into features and labels
    features, labels, target_masks, padding_masks, IDs = zip(*batch)

    # Ensure features are converted to tensors and are 2D
    features = [torch.tensor(f) if not isinstance(f, torch.Tensor) else f for f in features]

    # Reshape features to ensure they are 2D
    features = [f.unsqueeze(0) if f.dim() == 1 else f for f in features]
    features = [f.unsqueeze(0) if f.dim() == 2 else f for f in features]
    features = torch.cat(features, dim=0)  # Concatenate along the batch dimension

    # Initialize the tensor for the collated data
    collated_features = torch.zeros(len(features), max_len, features[0].size(-1))  # (batch_size, max_len, feature_dim)

    for i, feature in enumerate(features):
        end = min(feature.size(0), max_len)  # Ensure we do not exceed max_len
        collated_features[i, :end, :] = feature[:end, :]  # Assign the features

    # Convert labels to a tensor
    labels = torch.tensor(labels)
    target_masks = torch.stack(target_masks)  # Stack target masks

    # Create padding masks based on the collated features
    padding_masks = (collated_features.sum(dim=-1) == 0)  # Create padding mask where features are zero
    padding_masks = padding_masks.to(torch.bool)  # Ensure it's a boolean tensor

    IDs = list(IDs)  # Convert IDs to a list if needed

    # Ensure the return structure is correct
    return collated_features, labels, padding_masks, IDs  # Return exactly four elements

# New code starts here
# You can add any additional logic or modifications below this line
...
class ClassiregressionDataset(Dataset):
    ...
    def __getitem__(self, ind):
        """
        For a given integer index, returns the corresponding (seq_length, feat_dim) array and a label.
        Args:
            ind: integer index of sample in dataset
        Returns:
            X: (seq_length, feat_dim) tensor of the multivariate time series corresponding to a sample
            y: (num_labels,) tensor of labels (num_labels > 1 for multi-task models) for each sample
            target_masks: (seq_length, feat_dim) tensor of masks (if applicable)
            padding_masks: (seq_length) tensor indicating padding (if applicable)
            ID: ID of sample
        """
        ...
        # Extract the label corresponding to the sample
        y = self.labels_df.loc[self.IDs[ind]].values  # Assuming labels are stored in labels_df

        # Create target masks and padding masks (if applicable)
        target_masks = torch.ones(X.shape, dtype=torch.bool)  # Example: all ones, adjust as needed
        padding_masks = torch.zeros(X.shape[0], dtype=torch.bool)  # Example: all zeros, adjust as needed

        return torch.from_numpy(X), torch.tensor(y), target_masks, padding_masks, self.IDs[ind]  # Return all five elements
...
def collate_unsuperv(data, max_len=None, mask_compensation=False):
    ...
    # Convert features to a list to allow item assignment
    ...
    for i in range(batch_size):
        ...
        # Ensure features[i] is at least 2D
        if features[i].ndim == 1:
            features[i] = features[i].unsqueeze(0)  # Convert 1D to 2D (1 sample, n features)
...
def noise_mask(X, masking_ratio, mean_mask_length=3, mode='separate', distribution='geometric', exclude_feats=None):
    """
    ...
    Args:
        ...
        mean_mask_length: average length of masking subsequences (streaks of 0s). Used only when `distribution` is 'geometric'.
        mode: whether each variable should be masked separately ('separate'), or all variables at a certain positions
            should be masked concurrently ('concurrent')
        distribution: whether each mask sequence element is sampled independently at random, or whether
            sampling follows a markov chain (and thus is stateful), resulting in geometric distributions of
            masked squences of a desired mean length `mean_mask_length`
    ...
    if X.ndim == 1:
        X = X.reshape(1, -1)  # Reshape to (1, n_features) if it's 1D
    elif X.ndim == 0:
        raise ValueError("Input data X is empty. Please provide valid input data.")
        ...
        if mode == 'separate':  # each variable (feature) is independent
            ...
            for m in range(X.shape[1]):  # feature dimension
                if exclude_feats is None or m not in exclude_feats:
                    mask[:, m] = geom_noise_mask_single(X.shape[0], mean_mask_length, masking_ratio)  # time dimension
        else:  # replicate across feature dimension (mask all variables at the same positions concurrently)
            mask = np.tile(np.expand_dims(geom_noise_mask_single(X.shape[0], mean_mask_length, masking_ratio), 1), X.shape[1])
