...
# In AI-project/mvts_transformer/src/datasets/data.py

class MachineData(BaseData):
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        self.set_num_processes(n_proc=n_proc)

        # Initialize limit_size as an instance variable
        ...
        # Set max_seq_len based on config
        if config and 'data_window_len' in config and config['data_window_len'] is not None:
            self.max_seq_len = config['data_window_len']
        else:
            self.max_seq_len = 30  # Default value if not provided

        if self.limit_size is not None:
            if self.limit_size > 1:
                self.limit_size = int(self.limit_size)
            else:  # interpret as proportion if in (0, 1]
                self.limit_size = int(self.limit_size * len(self.all_IDs))
            self.all_IDs = self.all_IDs[:self.limit_size]
            self.all_df = self.all_df.loc[self.all_IDs]
        
        self.feature_df = self.all_df[['Volume', 'Closing Price', 'SMA', 'EMA', 'MACD']]
        self.labels_df = self.all_df[['Trend']]  # Assuming 'Trend' is your target variable

        # Define class_names based on unique values in the 'Trend' column
        self.class_names = sorted(self.all_df['Trend'].unique().tolist())  # Ensure class names are sorted
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        all_df = pd.concat(pd.read_csv(path) for path in input_paths)

        # Optionally limit the size of the DataFrame
        if self.limit_size is not None:
            all_df = all_df.head(self.limit_size)
        
        return all_df

    def load_single(self, filepath):
        """Loads a single CSV file and processes it to extract features and labels."""
        
        # Read the CSV file into a DataFrame
        df = pd.read_csv(filepath)

        # Check if the required columns are present
        required_columns = ['Volume', 'Closing Price', 'SMA', 'EMA', 'MACD', 'Trend']
        if not all(col in df.columns for col in required_columns):
            raise ValueError(f"Missing one of the required columns: {required_columns}")

        # Extract features and labels
        features = df[['Volume', 'Closing Price', 'SMA', 'EMA', 'MACD']]
        labels = df['Trend']

        # Convert labels to a categorical type and store class names
        labels = pd.Series(labels, dtype="category")
        self.class_names = labels.cat.categories.tolist()  # Store class names

        # Create a DataFrame for labels
        labels_df = pd.DataFrame(labels.cat.codes, dtype=np.int8)  # Convert to int8 for compatibility

        # Handle missing values in features
        num_nan = features.isna().sum().sum()
        if num_nan > 0:
            logger.warning(f"{num_nan} NaN values in features will be replaced by 0")
            features = features.fillna(0)

        # Set max_seq_len based on the number of samples
        self.max_seq_len = len(features)
